				...
				$form.find(':input').each(function() {
					...
					if ($field.hasClass(options.requiredClass) || ($field.attr('type') !== undefined && $field.attr('type').toLowerCase() != 'radio' && $field.attr('type').toLowerCase() != 'checkbox' && $field.val().length > 0)) {
						...
						if ($field.hasClass('iqs-form-phone-number')) {
							...
							regex = /^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/;
							msg = '3.9';
						} else if ($field.hasClass('iqs-form-sat')) {
                            regex = /^([1-8][0-9]{2}|9[0-8][0-9]|99[0-9]|1[0-9]{3}|2[0-3][0-9]{2}|2400)$/;
                            msg = '100-2400';
                        }
                        else if ($field.hasClass('ids-form-act-score')) {
                            regex = /^([1-9]|[12]\d|3[0-6])$/;
                            msg = '23';
                        }
                            ...
                            regex = /^([1-9]|[12]\d|3[0-6])$/;
                            msg = '23';
                        } else if ($field.hasClass('iqs-form-gpa')) {
                            regex = /^([0-3]\.\d|4\.0)$/;
                            msg = '23';
                        } else if ($field.hasClass('iqs-form-sat')) {
                            regex = /^([1-8][0-9]{2}|9[0-8][0-9]|99[0-9]|1[0-9]{3}|2[0-3][0-9]{2}|2400)$/;
							...
							using = '';
						} else if ($field.hasClass('ids-form-dob')) {
                            regex = /^\d{4}-\d{2}-\d{2}$/;
                            msg = '2000-05-05';
                        } else {
							using = '';
